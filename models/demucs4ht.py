...
from demucs.hdemucs import pad1d, ScaledEmbedding, MultiWrap, HEncLayer, HDecLayer


class HTDemucs2(nn.Module):
    """
...
class HTDemucs(nn.Module):
    ...
    def _ispec(self, z, length=None):
        ...
        # Get shape from real view to avoid complex_shape operation  
        *other, freqs, frames = z.shape
        n_fft = 2 * freqs - 2
        z = z.view(-1, freqs, frames)
        ...
        print(f"other dimensions: {other}")
        # Store the last dimension to avoid dynamic shape issues in reshape
        x_last_dim = x.shape[-1]
        # Use the original dimensions instead of hardcoding
        x = x.view(*other, x_last_dim)
    ...
    def _mask(self, z, m):
        ...
        if self.cac:
            B, S, C, Fr, T = m.shape # B, S, C, Fr, T
            #  print(m.shape, "M")
            out = m.reshape(B*S, -1, 2, Fr, T) ## becomes B*S, C/2, 2, Fr, T
            # print(out.shape, "OUT")
    ...
    def preprocess_istft_input(self, z, length, hl):
        # pad input to 44100
        z = torch.tensor(z)
        z = F.pad(z, (0, 0, 0, 1))
        z = F.pad(z, (2, 2))
    ...
    def forward(self, mix):
        ...
        # # Dictionary to store intermediate states
        intermediates = {}

        # print(f"mix dtype: {mix.dtype}")
        
        # z = self._spec(mix)
        # make z_real and z_imag go from (2, 2049, 44100) to (2, 2048, 44100)
        # z_real = z_real.unsqueeze(0)
        # z_imag = z_imag.unsqueeze(0)

        # # need to switch [1, 2*batch_size, 2048, 44100] to [batch_size, 2, 2048, 44100]
        # z_real = z_real.view(batch_size, 2, *z_real.shape[2:])
        # z_imag = z_imag.view(batch_size, 2, *z_imag.shape[2:])
        ...
        intermediates['state_1_after_stft'] = z.clone()
        print("made it past spec")
        
        mag = self._magnitude(z)
        x = mag
        # Ensure tensor maintains the model's precision after STFT
        x = x.to(torch.float32)
        intermediates['state_2_after_magnitude'] = mag.clone()
        ...
        # unlike previous Demucs, we always normalize because it is easier.
        # Manually compute std to avoid CoreML precision issues
        # mean = x.mean(dim=(1,2,3), keepdim=True).to(torch.float32)
        # centered = (x - mean).to(torch.float32)
        # variance = (centered ** 2).mean(dim=(1,2,3), keepdim=True).to(torch.float32)
        # epsilon = torch.tensor(1e-5, device=x.device, dtype=torch.float32)
        # std = torch.sqrt(variance + epsilon).to(torch.float32)
        # x = centered / std
        mean = x.mean(dim=(1, 2, 3), keepdim=True)
        std = x.std(dim=(1, 2, 3), keepdim=True)
        x = (x - mean) / (1e-5 + std)
        ...
        # Prepare the time branch input.
        intermediates['state_3_after_freq_norm'] = x.clone()
        ...
        stdt = xt.std(dim=(1, 2), keepdim=True)
        xt = (xt - meant) / (1e-5 + stdt)
        # Convert final result to model precision
        # xt = xt.to(model_dtype)
        intermediates['state_4_after_time_norm'] = xt.clone()

        print("made it past freq and time norm")
        ...
        for idx, decode in enumerate(self.decoder):
            ...
            if idx >= offset:
                ...
                if tdec.empty:
                    ...
                    ## freq is false so do [2:-4] slicing here
                xt = xt.contiguous()
                # if idx != 1:
                #     xt = xt[..., 2:-4]
                # else:
        ...
        xt = xt * stdt[:, None] + meant[:, None]
        z_real = self.preprocess_istft_input(zout.real, length, self.hop_length)
        z_imag = self.preprocess_istft_input(zout.imag, length, self.hop_length)

        return xt, z_real, z_imag
